...
| --- | --- | --- | --- | --- | --- | --- |
| CLI fetch | Snowflake fetch | --region -> account mapping -> snowsql PATH executable + externalbrowser | US oua13326; AU pn30490.ap-southeast-2; warehouse reporting; configured session database data_science/schema public, but SQL reads fully qualified DATA_SCIENCE.PREPROCESSING.LANTERN_RESPONSES | Snowflake and authenticated local operator; temporary CSV file then selected outfile/stdout | External-browser authentication; Snowflake configured role; subprocess argument vector | item_estimation/fetch.py:122; item_estimation/fetch.py:130; item_estimation/fetch.py:132; item_estimation/main.py:61 |
...
- CLI region chooses one of two hardcoded Snowflake accounts; snowsql executable from PATH receives a constructed argument vector, externalbrowser authentication, reporting warehouse and data_science/public session defaults; fully qualified SQL targets DATA_SCIENCE.PREPROCESSING.LANTERN_RESPONSES, so public does not select the accessed table (item_estimation/fetch.py:37, item_estimation/fetch.py:76, item_estimation/fetch.py:105). No shell=True path; Snowflake role privileges remain externally configured (item_estimation/fetch.py:122; item_estimation/fetch.py:132; item_estimation/fetch.py:161).
...
| P2 | Adversarial or incorrect CSV silently changes calibration results. | Untrusted producer supplies inference input and operator later relies on results. | Integrity loss in local analytics; downstream product effect requires separate import/publication workflow. | Curriculum filter and preprocessing; configured iteration count; min_obs is a minimum group-size filter, not an observation cap, and pd.read_csv loads the complete input. | Enforce input-byte/row budgets for untrusted workloads; validate provenance, required fields and result plausibility before downstream use; maintain source/result linkage. | item_estimation/main.py:67; item_estimation/load_data.py:60; item_estimation/run_inference.py:345 |
| P2 | Executable resolution or trusted pickle helper turns file control into local process execution. | Attacker can replace snowsql on operator PATH, or a caller actually invokes knowledge_graph with attacker-written pickle. | Code runs with operator authority, potentially reaching local data/authentication context. | shutil.which checks availability only and its resolved path is discarded; bare snowsql is resolved again by subprocess. Argument vectors avoid shell parsing, not binary substitution; no code-level executable provenance check; current main infer path does not call pickle helper. | Resolve and validate a trusted executable path and protect it from replacement; only load trusted pickles or use data-only serialization when helper is required. | item_estimation/fetch.py:125; item_estimation/fetch.py:161; item_estimation/load_data.py:49; item_estimation/load_data.py:230 |
| P3 | Configuration/output suffix sends files to unexpected locations or overwrites local outputs. | Ordinary reruns reuse the same suffix, including default -, and overwrite fixed outputs; unintended config/suffix is another path. Destination is writable. | Local data loss or output confidentiality drift. | Explicit CLI/config inputs; result directory created under resolved path; no remote path parameter. | Use unique run directories or explicit overwrite protection; treat config/suffix as trusted filesystem inputs and verify every output, including cwd difficulties_{suffix}.csv opened with w (item_estimation/run_inference.py:226), before automation. | item_estimation/main.py:99; item_estimation/main.py:146; item_estimation/run_inference.py:225; item_estimation/run_inference.py:348 |
